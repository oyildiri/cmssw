...
# with command line options: harvest --scenario pp --filetype DQM --conditions auto:run2_mc --mc -s HARVESTING:@trackingOnlyValidation+@trackingOnlyDQM+postProcessorHLTtrackingSequence -n -1 --filein file:/eos/home-c/cericeci/PV/forPSI_v2/CMSSW_15_0_4/src/RecoVertex/PrimaryVertexProducer_Alpaka/test/TTNewWithp0p5/2c01bfb3-a5cd-473c-86f9-af42707fcf00_output_DQM.root --python_file harvester.py --no_exec
...
options.register ('inputFileList',
                  ...
                  "Text file with local files")
# setup any defaults you want
options.outputFile = 'output.root'
options.inputFiles = []#'input.root']
options.inputFileList = None
options.maxEvents = -1 # -1 means all events
options.parseArguments()
if options.inputFileList:
    options.inputFiles = []
    for f in open(options.inputFileList, "r").readlines():
        options.inputFiles.append(f.replace("\n",""))
...
# Schedule definition
process.schedule = cms.Schedule(process.postProcessorHLTtrackingSequence_step,process.postValidation_trackingOnly_step,process.DQMHarvestTracking_step,process.dqmsave_step)
