#ifndef RecoVertex_PrimaryVertexProducer_WeightedMeanFitter_GMM_h
#define RecoVertex_PrimaryVertexProducer_WeightedMeanFitter_GMM_h
...
namespace WeightedMeanFitterGMM {
...
class WeightedMeanPrimaryVertexEstimator_GMM : public PrimaryVertexFitterBase {
public:
  WeightedMeanPrimaryVertexEstimator_GMM() = default;
  ~WeightedMeanPrimaryVertexEstimator_GMM() override = default;
    ...
    for (auto& cluster : clusters) {
      if (cluster.originalTracks().size() > 1) {
        ...
        if (useBeamConstraint && (tracklist.size() > 1)) {
          ...
          TransientVertex v = WeightedMeanFitterGMM::weightedMeanOutlierRejectionBeamSpot(points, tracklist, beamSpot);
          if (!v.hasTrackWeight()) {
          ...
          if ((v.positionError().matrix())(2, 2) != (WeightedMeanFitterGMM::startError * WeightedMeanFitterGMM::startError))
            pvs.push_back(v);
          ...
          TransientVertex v = WeightedMeanFitterGMM::weightedMeanOutlierRejection(points, tracklist);
          if (!v.hasTrackWeight()) {
            // if the fitter doesn't provide weights, fill dummy values
            TransientVertex::TransientTrackToFloatMap trkWeightMap;
            for (const auto& trk : v.originalTracks()) {
